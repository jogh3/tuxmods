...
export function getGamePath(api) {
...
export async function globalProfilePath(api) {
...
export const getPlayerProfiles = (() => {
    let cached = [];
    try {
        cached = fs.readdirSync(profilesPath())
...
export function gameSupportsProfile(gameVersion) {
    return semver.lt(semver.coerce(gameVersion), '4.1.206');
}
export async function getOwnGameVersion(state) {
    const discovery = selectors.discoveryByGame(state, GAME_ID);
    return await util.getGame(GAME_ID).getInstalledVersion(discovery);
}
export async function getActivePlayerProfile(api) {
...
export function parseModNode(node) {
...
export function getLatestInstalledLSLibVer(api) {
    const state = api.getState();
    const mods = util.getSafe(state, ['persistent', 'mods', GAME_ID], {});
    return Object.keys(mods).reduce((prev, id) => {
        if (mods[id].type === 'bg3-lslib-divine-tool') {
            const arcId = mods[id].archiveId;
            const dl = util.getSafe(state, ['persistent', 'downloads', 'files', arcId], undefined);
...
export async function getDefaultModSettingsFormat(api) {
    if (_FORMAT !== null) {
        return _FORMAT;
    }
    _FORMAT = 'v8';
...
export async function getDefaultModSettings(api) {
    if (_FORMAT === null) {
...
export async function convertToV8(someXml) {
...
export async function convertV6toV7(v6Xml) {
...
export function getLatestLSLibMod(api) {
    const state = api.getState();
...
export async function extractPakInfoImpl(api, pakPath, mod, isListed) {
...
export async function extractMeta(api, pakPath, mod) {
...
let storedLO = [];
export async function writeModSettings(api, data, bg3profile) {
    if (!bg3profile) {
        return;
    }
...
export async function parseLSXFile(lsxPath) {
    const dat = await fs.readFileAsync(lsxPath, { encoding: 'utf8' });
    return parseStringPromise(dat);
}
export async function readModSettings(api) {
...
export async function readStoredLO(api) {
    ...
    const mods = util.getSafe(state, ['persistent', 'mods', GAME_ID], {});
    const enabled = Object.keys(mods).filter(id => util.getSafe(vProfile, ['modState', id, 'enabled'], false));

...
export async function serialize(context, loadOrder, profileId) {
...
export async function deserialize(context) {
...
export async function importFromBG3MM(context) {
...
export async function importModSettingsFile(api) {
    const state = api.getState();
    const profileId = selectors.activeProfile(state)?.id;
    const options = {
...
export async function importModSettingsGame(api) {
...
async function processBG3MMFile(api, jsonPath) {
    const state = api.getState();
    const profileId = selectors.activeProfile(state)?.id;
...
export async function getNodes(lsxPath) {
...
export async function processLsxFile(api, lsxPath) {
    const state = api.getState();
    const profileId = selectors.activeProfile(state)?.id;
    api.sendNotification({
        id: NOTIF_IMPORT_ACTIVITY,
    ...
    try {
        ...
        let uuidArray = loNode?.children !== undefined
...
export async function exportToFile(api) {
    let selectedPath;
...
export async function exportToGame(api) {
    const bg3ProfileId = await getActivePlayerProfile(api);
    const settingsPath = path.join(profilesPath(), bg3ProfileId, 'modsettings.lsx');
    logDebug(`exportToGame ${settingsPath}`);
    exportTo(api, settingsPath);
}
export async function deepRefresh(api) {
    const state = api.getState();
    const profileId = selectors.activeProfile(state)?.id;
    // get load order from state
    const loadOrder = util.getSafe(api.getState(), ['persistent', 'loadOrder', profileId], []);
...
export async function validate(prev, current) {
...
export function genProps(context, profileId) {
    const api = context.api;
    const state = api.getState();
...
export async function ensureLOFile(context, profileId, props) {
    if (props === undefined) {
...
export function loadOrderFilePath(profileId) {

...
import { GAME_ID, IGNORE_PATTERNS, MOD_TYPE_BG3SE, MOD_TYPE_LOOSE, MOD_TYPE_LSLIB, MOD_TYPE_REPLACER, } from './common';
import * as gitHubDownloader from './githubDownloader';
import Settings from './Settings';
import reducer from './reducers';
import { migrate } from './migrations';
import { logDebug, forceRefresh, getLatestInstalledLSLibVer, getGameDataPath, getGamePath, globalProfilePath, modsPath, getLatestLSLibMod, getOwnGameVersion, readStoredLO, getDefaultModSettings, getDefaultModSettingsFormat, getActivePlayerProfile, profilesPath, convertV6toV7, convertToV8, } from './util';
import { testLSLib, testBG3SE, testEngineInjector, testModFixer, testReplacer, installLSLib, installBG3SE, installEngineInjector, installModFixer, installReplacer, } from './installers';
import { isBG3SE, isLSLib, isLoose, isReplacer, } from './modTypes';
import { deserialize, importModSettingsFile, importModSettingsGame, importFromBG3MM, serialize, exportToGame, exportToFile, validate, getNodes, } from './loadOrder';
...
async function prepareForModding(api, discovery) {
    ...
    return fs.statAsync(mp)
        .catch(() => fs.ensureDirWritableAsync(mp, () => Bluebird.resolve()))
...
function main(context) {
    ...
    context.registerModType(MOD_TYPE_LSLIB, 15, (gameId) => gameId === GAME_ID, () => undefined, isLSLib, { name: 'BG3 LSLib', noConflicts: true });
    ...
    context.registerLoadOrder({
        ...
        usageInstructions: (() => (_jsx(InfoPanelWrap, { api: context.api, getOwnGameVersion: getOwnGameVersion, readStoredLO: readStoredLO, installLSLib: onGameModeActivated, getLatestLSLibMod: getLatestLSLibMod }))),
    });
    const isBG3 = () => {
        const state = context.api.getState();

...
class MasterChiefCollectionGame {
    ...
    requiresLauncher = util.toBlue((gamePath, store) => this.checkLauncher(gamePath, store));
...
module.exports = {
    default: (context) => {
        ...
        //   return collator;
        // };
        context.registerModType(MODTYPE_PLUG_AND_PLAY, 15, (gameId) => gameId === GAME_ID, () => undefined, testPlugAndPlayModType, {
        ...
        context.registerTableAttribute('mods', {
            ...
            customRenderer: (mod) => {
                ...
                return React.createElement(FlexLayout, { type: 'row' }, React.createElement(FlexLayout.Flex, { className: 'haloimglayout' }, haloEntries.map((entry, idx) => createImgDiv(entry, idx))));
            },
            calc: (mod) => util.getSafe(mod, ['attributes', 'haloGames'], undefined),
            filter: new OptionsFilter([].concat([{ value: OptionsFilter.EMPTY, label: '<None>' }], Object.keys(HALO_GAMES)
                .map(key => {

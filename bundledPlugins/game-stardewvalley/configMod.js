...
export function registerConfigMod(context) {
...
const shouldSuppressSync = (api) => {
    ...
    const isActivityRunning = (activity) => util.getSafe(state, ['session', 'base', 'activity', activity], []).length > 0;
...
export async function addModConfig(api, files, modsPath) {
...
export async function ensureConfigMod(api) {
    const state = api.getState();
...
export async function onWillEnableMods(api, profileId, modIds, enabled, options) {
    const state = api.getState();
...
export async function applyToModConfig(api, cb) {
...
export async function onRevertFiles(api, profileId) {
    const state = api.getState();
    const profile = selectors.profileById(state, profileId);
    if (profile?.gameId !== GAME_ID) {
        return;
...
export async function onAddedFiles(api, profileId, files) {
...
//# sourceMappingURL=configMod.js.map
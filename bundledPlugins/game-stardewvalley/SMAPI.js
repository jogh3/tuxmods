...
export async function deploySMAPI(api) {
...
export async function downloadSMAPI(api, update) {
    ...
    try {
        ...
        const dlId = await util.toPromise(cb => api.events.emit('start-download', [nxmUrl], dlInfo, undefined, cb, undefined, { allowInstall: false }));
        const modId = await util.toPromise(cb => api.events.emit('start-install-download', dlId, { allowAutoEnable: false }, cb));

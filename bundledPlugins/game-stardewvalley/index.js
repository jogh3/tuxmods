...
const path = require('path'), { clipboard } = require('electron'), rjson = require('relaxed-json'), { SevenZip } = util, { deploySMAPI, downloadSMAPI, findSMAPIMod } = require('./SMAPI'), { GAME_ID, _SMAPI_BUNDLED_MODS, getBundledMods, MOD_TYPE_CONFIG } = require('./common');
...
function init(context) {
    ...
    context.registerInstaller('stardew-valley-installer', 50, testSupported, (files, destinationPath) => Bluebird.resolve(install(context.api, dependencyManager, files, destinationPath)));
    context.registerModType('SMAPI', 30, gameId => gameId === GAME_ID, getSMAPIPath, isSMAPIModType);
    context.registerModType(MOD_TYPE_CONFIG, 30, (gameId) => (gameId === GAME_ID), () => path.join(getDiscoveryPath(), defaultModsRelPath()), () => Bluebird.resolve(false));
    context.registerModType('sdvrootfolder', 25, (gameId) => (gameId === GAME_ID), () => getDiscoveryPath(), (instructions) => {
        ...
        const hasModsFolder = copyInstructions.find(instr => instr.destination.startsWith(defaultModsRelPath() + path.sep)) !== undefined;
    ...
    */
    context.registerTest('sdv-incompatible-mods', 'gamemode-activated', () => Bluebird.resolve(testSMAPIOutdated(context.api, dependencyManager)));
    context.once(() => {

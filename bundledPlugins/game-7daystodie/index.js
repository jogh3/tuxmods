...
async function prepareForModding(context, discovery) {
    const isUDFSet = util.getSafe(context.api.getState(), ['settings', '7daystodie', 'udf'], undefined) != null;
...
async function installContent(files, destinationPath, gameId) {
    ...
    return getModName(path.join(destinationPath, modFile))
        ...
        const filtered = files.filter(filePath => filePath.startsWith(rootPath) && !filePath.endsWith(path.sep));
...
function InfoPanel(props) {
    ...
    return (_jsxs("div", { style: { display: 'flex', flexDirection: 'column', padding: '16px' }, children: [_jsxs("div", { style: { display: 'flex', whiteSpace: 'nowrap', alignItems: 'center' }, children: [t('Current Prefix Offset: '), _jsx("hr", {}), _jsx("label", { style: { color: 'red' }, children: currentOffset })] }), _jsx("hr", {}), _jsx("div", { children: t('7 Days to Die loads mods in alphabetic order so Vortex prefixes '
...
function InfoPanelWrap(props) {
    ...
    const currentOffset = useSelector((state) => makePrefix(util.getSafe(state, ['settings', '7daystodie', 'prefixOffset', profileId], 0)));
    return (_jsx(InfoPanel, { t: api.translate, currentOffset: currentOffset }));
}
function main(context) {
    ...
    context.registerLoadOrder({
        ...
        usageInstructions: (() => {
            ...
            return (_jsx(InfoPanelWrap, { api: context.api, profileId: profileId }));
        }),
    });

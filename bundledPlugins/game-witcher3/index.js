...
import { DO_NOT_DEPLOY, GAME_ID, getLoadOrderFilePath, LOCKED_PREFIX, SCRIPT_MERGER_ID } from './common';
...
import { installContent, installMenuMod, installTL, installDLCMod, installMixed, scriptMergerDummyInstaller, scriptMergerTest, testMenuModRoot, testSupportedContent, testSupportedTL, testSupportedMixed, testDLCMod } from './installers';
import { W3Reducer } from './reducers';
import { getDLCPath, getAllMods, determineExecutable, getDocumentsPath, getTLPath, isTW3, notifyMissingScriptMerger } from './util';
import TW3LoadOrder from './loadOrder';
import { onDidDeploy, onDidPurge, onDidRemoveMod, onGameModeActivation, onModsDisabled, onProfileWillChange, onSettingsChange, onWillDeploy } from './eventHandlers';
...
function findGame() {
    try {
        const instPath = winapi.RegGetValue('HKEY_LOCAL_MACHINE', 'Software\\CD Project Red\\The Witcher 3', 'InstallFolder');
        if (!instPath) {
...
function prepareForModding(api) {
    return (discovery) => {
        ...
        const ensurePath = (dirpath) => fs.ensureDirWritableAsync(dirpath)
...
function main(context) {
    ...
    context.registerModType('w3modlimitpatcher', 25, isTW3(context.api), getTLPath(context.api), () => Bluebird.resolve(false), { deploymentEssential: false, name: 'Mod Limit Patcher Mod Type' });

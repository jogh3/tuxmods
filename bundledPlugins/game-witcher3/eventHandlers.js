...
import { GAME_ID, getPriorityTypeBranch, PART_SUFFIX, INPUT_XML_FILENAME, SCRIPT_MERGER_ID, I18N_NAMESPACE } from './common';
...
export function onGameModeActivation(api) {
...
export const onModsDisabled = (api, priorityManager) => {
...
export const onDidRemoveMod = (api, priorityManager) => {
...
export const onDidPurge = (api, priorityManager) => {
    return async (profileId, deployment) => {
        const state = api.getState();
        const activeProfile = validateProfile(profileId, state);
...
export const onProfileWillChange = (api) => {
...
export const onSettingsChange = (api, priorityManager) => {

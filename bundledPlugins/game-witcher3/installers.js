...
export function testMenuModRoot(instructions, gameId) {
...
export function installMenuMod(files, destinationPath) {
    ...
    const modFiles = otherFiles.filter(file => file.toLowerCase().split(path.sep).includes('mods'));
    ...
    const inputInstructions = uniqueInput.map(file => toCopyInstruction(file, path.join(inputFileDestination, path.basename(file))));
...
export function testSupportedContent(files, gameId) {
...
export function installContent(files, destinationPath) {
...
export function testSupportedTL(files, gameId) {
    const supported = (gameId === GAME_ID)
...
export function installTL(files) {
...
export function testDLCMod(files, gameId) {
    if (gameId !== GAME_ID) {
...
export function installDLCMod(files) {
    const modNames = [];
...
export function testSupportedMixed(files, gameId) {
    if (gameId !== GAME_ID) {
        return Promise.resolve({ supported: false, requiredFiles: [] });
    }
...
export function installMixed(files) {

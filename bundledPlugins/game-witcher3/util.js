...
export async function getDeployment(api, includedMods) {
    ...
    const deployment = await modTypes.reduce(async (accumP, modType) => {
        ...
        try {
            const manifest = await util.getManifest(api, modType, GAME_ID);
...
export const getDocumentsPath = (game) => {
    return path.join(util.getVortexPath('documents'), 'The Witcher 3');
};
export const getDLCPath = (api) => {
    return (game) => {
...
export const hasPrefix = (prefix, fileEntry) => {
...
export async function findModFolders(installationPath, mod) {
    ...
    await turbowalk(path.join(installationPath, mod.installationPath), (entries) => {
...
export async function getManagedModNames(api, mods) {
...
export async function getAllMods(api) {
...
export async function getManuallyAddedMods(api) {
    const state = api.getState();
    const discovery = util.getSafe(state, ['settings', 'gameMode', 'discovered', GAME_ID], undefined);
...
export function isLockedEntry(modName) {
...
export function determineExecutable(discoveredPath) {
...
export function validateProfile(profileId, state) {
...
;
export function isXML(filePath) {
    return ['.xml'].includes(path.extname(filePath).toLowerCase());
}
export function isSettingsFile(filePath) {

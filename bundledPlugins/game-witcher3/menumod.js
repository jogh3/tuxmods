...
async function getExistingCache(state, activeProfile) {
    ...
    try {
        const cacheData = await fs.readFileAsync(path.join(stagingFolder, mod.installationPath, CACHE_FILENAME), { encoding: 'utf8' });
...
function populateCache(api, activeProfile, modIds, initialCacheValue) {
    ...
    const stagingFolder = selectors.installPathForGame(state, GAME_ID);
    return Bluebird.reduce(enabledMods, (accum, mod) => {
...
export async function onWillDeploy(api, deployment, activeProfile) {
    ...
    const newCache = await Bluebird.reduce(keys, async (accum, key) => {
        if (docFiles.find(matcher) !== undefined) {
            ...
            await Bluebird.each(fileMap[key], async (iter) => {
...
export async function ensureMenuMod(api, profile) {
    ...
    if (mod === undefined) {
        ...
        api.store.dispatch(actions.setModAttribute(profile.gameId, modName, 'name', 'Witcher 3 Menu Mod'));
        api.store.dispatch(actions.setModAttribute(profile.gameId, modName, 'type', 'witcher3menumoddocuments'));
        api.store.dispatch(actions.setModAttribute(profile.gameId, modName, 'logicalFileName', 'Witcher 3 Menu Mod'));
...
//# sourceMappingURL=menumod.js.map
...
import { GAME_ID, getLoadOrderFilePath, MERGE_INV_MANIFEST, SCRIPT_MERGER_ID, W3_TEMP_DATA_DIR, MergeDataViolationError } from './common';
...
function genBaseProps(api, profileId, force) {
    ...
    const localMergedScripts = (force) ? true : util.getSafe(state, ['persistent', 'profiles', profileId, 'features', 'local_merges'], false);
...
function backupPath(profile) {
    return path.join(util.getVortexPath('userData'), profile.gameId, 'profiles', profile.id, 'backup');
}
...
export async function storeToProfile(api, profileId) {
...
export async function restoreFromProfile(api, profileId) {
    const props = genBaseProps(api, profileId);
    if (props === undefined) {
        return;
    }
...
export async function queryScriptMerges(api, includedModIds, collection) {
    const state = api.getState();
    ...
    if (optionalMods.length > 0 || diff.length !== 0) {
        throw new MergeDataViolationError(diff || [], optionalMods || [], util.renderModName(collection));
    }
}
export async function exportScriptMerges(api, profileId, includedModIds, collection) {
...
export async function importScriptMerges(api, profileId, fileData) {
    const props = genBaseProps(api, profileId, true);
    if (props === undefined) {
        return;
    }
...
export async function makeOnContextImport(api, collectionId) {
    const state = api.getState();
    const mods = util.getSafe(state, ['persistent', 'mods', GAME_ID], {});
...
//# sourceMappingURL=mergeBackup.js.map
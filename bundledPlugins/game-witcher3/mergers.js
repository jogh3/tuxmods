...
export const doMergeXML = (api) => async (modFilePath, targetMergeDir) => {
    try {
...
export const canMergeSettings = (api) => {
    return (game, gameDiscovery) => {
        if (game.id !== GAME_ID) {
            return undefined;
        }
...
export const doMergeSettings = (api) => async (modFilePath, targetMergeDir) => {
    // if (isSettingsMergeSuppressed(api)) {
    //   return Promise.resolve();
    // }
    try {
...
async function readSettingsFile(api, modFilePath, mergeDirPath) {
    const state = api.store.getState();
    const discovery = util.getSafe(state, ['settings', 'gameMode', 'discovered', GAME_ID], undefined);
    if (!discovery?.path) {
        return Promise.reject({ code: 'ENOENT', message: 'Game is not discovered' });

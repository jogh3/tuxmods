...
export const registerActions = (props) => {
    const { context } = props;
    ...
    context.registerAction('mods-action-icons', 300, 'start-install', {}, 'Import Script Merges', instanceIds => { makeOnContextImport(context.api, instanceIds[0]); }, instanceIds => {
        const state = context.api.getState();
    ...
    context.registerAction('mods-action-icons', 300, 'start-install', {}, 'Import Load Order', instanceIds => { importLoadOrder(context.api, instanceIds[0]); }, instanceIds => {
        const state = context.api.getState();
        const mods = util.getSafe(state, ['persistent', 'mods', GAME_ID], {});
        if (mods[instanceIds?.[0]]?.type !== 'collection') {
            return false;
...
//# sourceMappingURL=iconbarActions.js.map